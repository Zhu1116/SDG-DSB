...
class Diffusion():
    ...
    def ddpm_dps_sampling(self, steps, pred_x0_fn, x1, x1_pinv, x1_forw, 
        ...
        Endm = torch.from_numpy(x1_forw['E_hat']).float().to(self.device)
        o2_msi = torch.from_numpy(x1_forw['MSI']).float().to(self.device)
        o2_msi = o2_msi.permute([2, 0, 1]).unsqueeze(0)

        srf = torch.from_numpy(x1_forw['srf']).float().to(self.device)
        distances = []
        mus = []
        ...
        for prev_step, step in pair_steps:
            ...
            # turn on gradient
            xt.requires_grad_()
            pred_x0_1 = pred_x0_fn(xt[:, :3, :, :], step)
            pred_x0_2 = pred_x0_fn(xt[:, 3:6, :, :], step)

            # pred_x0 = pred_x0_1
            pred_x0 = torch.cat((pred_x0_1, pred_x0_2), 1)  # 2 models

            pred_hsi0 = torch.matmul(Endm, pred_x0.reshape(pred_x0.shape[1], -1))
            pred_hsi0_visual = pred_hsi0.reshape(pred_hsi0.shape[0], pred_x0.shape[2], pred_x0.shape[3]).unsqueeze(0)
            pred_msi = torch.matmul(srf, pred_hsi0).reshape(srf.shape[0], pred_x0.shape[2], pred_x0.shape[3]).unsqueeze(0)

            residual = pred_msi - o2_msi
            ...
            pair_steps.set_postfix({"mu_x0": mu_x0.item(), "distance": residual_norm.item()})

            # Adam
            t += 1
            m = beta1 * m + (1 - beta1) * norm_grad
            v = beta2 * v + (1 - beta2) * (norm_grad ** 2)
            m_hat = m / (1 - beta1 ** t)
            v_hat = v / (1 - beta2 ** t)
            if t < 5:   # warm up
                xt = xt - rho * norm_grad
            else:
                xt = xt - rho * m_hat / (torch.sqrt(v_hat) + epsilon)

            distances.append(residual_norm.item())
            mus.append(mu_x0.item())
            xt = xt.detach()

            if results_dir is not None and prev_step % 100 == 0:
                plt.imsave(str(results_dir / f"{prev_step}.png"), to_rgb(np.clip(np.transpose(((pred_hsi0_visual+1)/2).detach().cpu().squeeze().numpy(), (1, 2, 0)), 0, 1)))
